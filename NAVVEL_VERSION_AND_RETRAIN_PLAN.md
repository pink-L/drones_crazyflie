...
- ⚠️ **seed 11/12 是第二次跑**：第一次（`NavVel-P1-A2L3` 首批，`3yq8fhik`/`v0l4bqnv`，`failed`）被我**违反"GPU 工作必须串行"**的操作 OOM 杀掉 —— 当时我并发跑了一个 eval（§0.5.7 坑 9c）。**这是本会话我犯的第二个错**，记录在案。
- ⚠️ **保守之处（同 §0.5.15①）**：对 `A1b` 而言 `A2L3` 仍是"层数 4→3 **+** 逐柱 z 跨度随机"**两个变量**。

**② 验收（双口径，见 §0.6.6）**：`384×600`（对齐）+ `384×1500`（收口），各 × {filter ON, OFF}，**共 12 次 eval，全程串行**。
命令与产物：
- `/tmp/navvel_p1/acc_a2L3_600/`（`--num-envs 384 --steps 600 --parallel 2`）
- `/tmp/navvel_p1/acc_a2L3_1500/`（`--num-envs 384 --steps 1500 --parallel 1`；显存守卫自动降并发）
- 阶段日志 `/tmp/navvel_p1/acc/stage.log`

**③ 首批观察（`s11`，`384×600`）—— 速度指标确实抓到了到达率抓不到的东西**

| `s11` | `arrival@0.2` | 首达**中位**步 | 首达 p90 | 到达 env 数 | `intervened_step_frac` |
|---|---|---|---|---|---|
| filter **ON** | **0.8411** | 438.0 | 504.9 | **322** | 0.4032 |
| filter **OFF** | 0.7031 | **395.0** | 419.0 | 269 | 0.3952 |

⇒ **滤波器让无人机飞得更慢（中位 438 vs 395 步），但到达的 env 多 53 个（322 vs 269）**。
这正是"600 步 = 时间窗"会掩盖的效应：ON 的 p90 = 504.9 已逼近 600 步上限，**说明它的成功数被地平线截断了** ⇒ 1500 口径下这个差距会进一步放大。**"滤波器 = 保守减速"这个直觉被量化了。**

**④ 完整验收结果（待填）**：`384×600` / `384×1500` 各 seed 的 `arrival@0.2`、`arrival_steps_median/p90`、`dropped_relevant_step_frac`、`zero_intervention_rate`、碰撞数。
