...
| # | 决策项 | 用户决定 | 落地含义 |
|---|---|---|---|
| 1 | **验收协议** | **两套都跑** | 每个 ckpt 跑 **2 次** eval（filter ON/OFF 各 1）：<br>• **`512 × 600`** —— 与历史数字（**历史 A1a/A1b/A2 全是 512×600**）**同口径**，用于**阶梯对齐**；<br>• **`512 × 1500`** —— **收口口径**（= 1 个完整 episode），`arrival@0.2` 读作“**每 episode 成功率**”，用于**判过/不过**。<br>⚠️ **两个口径的数字永远不许混进同一张对照表**；报告必须写清 `rollout_steps`。<br>⚠️ **尺寸定为 512，不是 384**（见 §0.5.17② / 坑 9b-① / 坑 9d 更正）：512 曾因图形故障挂死，现已恢复；且证据显示 `num_envs` **不改变布局**，所以用 384 既无收益、又丢掉与历史的对齐。 |
...
#### 0.6.8 阶段 1a 阶梯的**同口径完整对照**（**512 envs**，2026-09-14 20:16）

**36 次 eval 全部 `exit=0`**（3 seed × {A1a,A1b,A2} × {600,1500} × {ON,OFF} = 36，加 A2L3 的 12 次共 48 次）。全部用 `acceptance_eval.py` 串行执行，产物在 `/tmp/navvel_p1/lad_*_512x*`。

**① `512 × 600`（对齐口径 —— 但注意它只是 40% episode 的**时间窗**）**

| profile | M | 柱连续性 | `arr@0.2` ON | `arr@0.2` OFF | med ON | med OFF | speed cost | 碰撞 ON/OFF | `drop_step` ON | 干预步 ON |
|---|---|---|---|---|---|---|---|---|---|---|
| `A1a` | 16 | — | 0.7865 | 0.6328 | 428.7 | 390.0 | 1.0991 | 0.00 / **0.67** | 0.0000 | 238.3 |
| `A1b` | 16 | ✅ 0.531 | 0.8053 | 0.5846 | 437.7 | 393.7 | 1.1118 | 0.00 / **0.33** | 0.0295 | 250.1 |
| `A2` | 24 | ❌ 1.443 | 0.8340 | 0.7246 | 439.0 | 395.3 | 1.1105 | 0.00 / 0.00 | **0.1886** | 241.6 |
| **`A2L3`** | **12** | ✅ 0.792 | 0.8034 | 0.7253 | 428.7 | 393.3 | **1.0898** | 0.00 / 0.33 | **0.0128** | 242.1 |

**② `512 × 1500`（收口口径 = 1 个完整 episode）**

| profile | M | 柱连续性 | `arr@0.2` ON | `arr@0.2` OFF | med ON | med OFF | speed cost | 碰撞 ON/OFF | `drop_step` ON | 干预步 ON |
|---|---|---|---|---|---|---|---|---|---|---|
| `A1a` | 16 | — | 0.9980 | 0.9811 | 433.3 | 391.0 | 1.1083 | 0.00 / 0.00 | 0.0000 | 255.2 |
| `A1b` | 16 | ✅ 0.531 | 0.9974 | 0.9779 | 442.3 | 394.3 | 1.1217 | 0.00 / 0.33 | 0.0118 | 264.6 |
| `A2` | 24 | ❌ 1.443 | 0.9987 | 0.9948 | 440.3 | 396.3 | 1.1110 | 0.00 / 0.00 | **0.1307** | 249.8 |
| **`A2L3`** | **12** | ✅ 0.792 | 0.9980 | 0.9961 | **430.7** | 394.7 | **1.0912** | 0.00 / 0.00 | **0.0051** | 254.4 |

**③ 门槛判定（收口口径 `512×1500`）—— 全部 4 档、全部 7 个门槛 PASS**

```
gate                              A1a    A1b     A2   A2L3
arrival@0.2_gate>=0.85           PASS   PASS   PASS   PASS
intervened_steps_gate<=300       PASS   PASS   PASS   PASS
zero_collision_ON_gate           PASS   PASS   PASS   PASS
zero_oob_gate                    PASS   PASS   PASS   PASS
h_min_train_gate>=0              PASS   PASS   PASS   PASS
stall_gate<=0.10                 PASS   PASS   PASS   PASS
dropped_relevant_frac_gate<0.01  PASS   PASS   PASS   PASS
```

**①⚠️ 重要读数**：**在收口口径下 `arrival@0.2` 已无区分度**（4 档全在 0.9974–0.9987）—— 整个阶梯都"及格"了。
**⇒ 所以阶段 1a 的收口不能再靠"过没过"，只能靠"哪个档更好"。** 判别的量是：

| 判别量（1500 口径） | 最优 | 次优 | 最差 | 结论 |
|---|---|---|---|---|
| **`filter speed cost`**（滤波器代价，越低越好） | **`A2L3` 1.0912** | `A1a` 1.1083 | `A1b` 1.1217 | ✅ **`A2L3` 最优**（滤波器对它的约束最小） |
| **`dropped_relevant_step_frac`**（观测窗口浪费） | `A1a` 0.0000 | **`A2L3` 0.0051** | `A2` **0.1307** | ✅ **`A2L3` 在"合法柱"里最优**（A1a 只是没柱） |
| **`arrival_steps_median` ON**（越快越好） | **`A2L3` 430.7** | `A1a` 433.3 | `A1b` 442.3 | ✅ **`A2L3` 最优** |
| 干预步数 ON | `A2` 249.8 | **`A2L3` 254.4** | `A1b` 264.6 | `A2L3` 次优 |

**④ `A2L3` 的结论**：它是 **4 档里唯一"3 项判别量里 3 项最优或次优"** 的档，而且是**唯一同时满足"柱连续 (L≥3)"与"M ≤ K=8"** 的档（§0.5.16⑥ 已证明两者只能在 L=3 同时成立）。
⇒ **阶段 1a 的正确收口是 `A2L3`，不是 `A2`**：`A2` 的 `arrival@0.2` 看着最高（0.8340 @600 / 0.9987 @1500），但那是**在一个柱中有 1.44×2r 缝的世界里**取得的，且它的 `dropped_relevant_step_frac` 是 `A2L3` 的 **26 倍**（0.1307 vs 0.0051）。
> **这正是我在 §0.5.16⑥ 立的连续性门禁的价值**：如果当初只看 `arrival@0.2`，会误选 `A2`。

**⑤ 速度代价的跨地平线漂移（`A2L3` 最稳）**

| profile | 600 | 1500 | 漂移 |
|---|---|---|---|
| `A1a` | 1.0991 | 1.1083 | +0.0091 |
| `A1b` | 1.1118 | 1.1217 | +0.0100 |
| `A2` | 1.1105 | 1.1110 | +0.0006 |
| **`A2L3`** | **1.0898** | **1.0912** | **+0.0014** |

⇒ 漂移 ≤1%（`A2L3` 只 0.14%），**远小于档间差距**（`A2L3` 1.0912 vs `A1b` 1.1217 = 3%）⇒ **该指标足以区分档位**，但它自己**没有可比的门槛**（1.10 落在 `A1a`/`A1b` 的取值为内）⇒ 已按你的决定改为**只记录 + WARN**（§0.6.7 B）。
