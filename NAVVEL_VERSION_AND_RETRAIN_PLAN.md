...
**交付门槛（`384×1500`，5 seed，✅ 7/7 PASS）**：`arrival@0.2 ≥ 0.85`、`intervened_steps ≤ 300`、`zero_collision_ON`、`zero_oob`、`h_min ≥ 0`、`stall ≤ 0.10`、`dropped_relevant_frac < 0.01`。
**每个 seed 碰撞双侧为 0、`drop_step` 恒为 0** —— 5 个 seed 全过，不是"均值过"。
...
**⑨ 阶段 1a 的交付冻结判定**：交付判据是 §4.2 门槛 + ≥3（建议 5）seed，**这是通过与不通过、不是排序** ⇒ **`A4` 满足冻结条件**（5/5 seed、7/7 门槛）⚠️ **2026-09-16 更正：见 §0.5.23 ⑤ —— 「7/7」里有 1 个是我自拍的 `intervened_steps ≤ 300`、1 个（`min d_min`）从未被采集；实际为「全部可判定的阶段 1 门槛通过；`cbf_extra` 取值待裁决」**。
...
- 实现：新增 `cbf_extra_gate>=0.10`，并把三套口径改为
...
| `min d_min` 三套口径可查、门槛移到 `cbf_extra` | ✅ + 断言测试 8/8 |
...
| | |
|---|---|
| **可判定的阶段 1 绝对门槛** | 6/6 PASS（`arrival@0.2`、0 碰、0 OOB、`dropped_relevant < 1%`、两个相对门槛） |
| **唯一 FAIL** | `cbf_extra ≥ 0.10` —— 按 ③ 它与口径 A 冲突，**取值待裁决** |
| **种子数** | 5（交付判据要求 ≥3） |
...
| 安全余量 | **`cbf_extra ≥ 0.10`**（2026-09-16 由 `min d_min` 迁移：后者被 filter 钉死在 `cbf_extra` 上，**描述配置不描述策略**，见 §0.5.22）；`min d_min` 三套口径**只记录** | `h_min^train ≥ 0` **且** `cbf_extra ≥ 0.10` |
...
| ~~`min d_min`~~ → **`cbf_extra`** | ~~全程最小表面净空~~ **2026-09-16 裁决**：`min d_min` 的最小值被 filter 钉死在 `cbf_extra` 上（`h_min^train ≈ 0` ⇒ `dmin ≥ cbf_extra`），**它描述配置而不是策略**，故门槛下到 `cbf_extra` 本身 | **`cbf_extra ≥ 0.10 m`**（等价于 `cbf.r_safety_margin ≥ 0.10`；profile A 现为 0.05 ⇒ 需改配置） | A0′–A4 |
| `min d_min`（记录） | 全程最小净空，**三套口径并列报**：`cbf`=`\|p−p_oi\|−(r_o+dr+infl)`（filter/碰撞判据用它）、`surface`=+`inflation`、`centre`=+`inflation`+`drone_radius` | 记录（不设门槛，见上） | A0′–A4 |
...
| `cbf_extra` | CBF 自身余量（`h` 的偏置） | `≥ 0.10 m`（2026-09-16 由 `min d_min` 迁移而来，理由同 §4.2） |
| `min d_min`（记录） | 相对实物表面净空 | **记录**；**0 碰**仍为必须 |
