...
- **H_p（软蒸馏有效）→ 证伪**：`dual-p0` ≈ `dual-p1`（p=0.764）。`p=0` 的"裸奔执行 + 罚偏离 CBF"
  ...
  `p0`，且绝对值离门槛还差 12 pt。
- **H_sign（贴边由 log 符号驱动）→ 部分成立**：符号修复（legacy→penalty）确实把 `min_clearance_global_min`
  （cbf 口径，ON）从"钉在 0.05"抬开——`dual-p0`=[0.059,0.0689,0.0501,0.0639,0.0696]、
  `dual-pann` 最高 **0.0836**、`fo-p0/pann` 0.051–0.059；而 `p1` 臂仍贴 0.05
  （`dual-p1`=[0.0503,0.0515,0.0552]、`fo-p1`=[0.052,0.0508,0.0507]）。**但零介入率不动** ⇒
  "贴边"确实由 legacy 的正 log 奖励驱动（现已修），**介入却由"朝障接近速度"（n·v+αh<0）驱动，
  不由距离驱动**——所以修符号只让策略离得稍远，没让它"更早减速/更切向通过"。
