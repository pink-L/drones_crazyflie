...
| **9b-①** | **✅ 绕过的办法：把验收的 env 数降下来**（**2026-09-14 实测，用户提出**）| 同一份 ckpt、同样 600 步，只改 `--num-envs`：**128 ✅ / 256 ✅ / 384 ✅ / 448 ✅ / 512 ❌**（512 报 544 次 RTX 失败并空转）。⇒ 该图形故障是“**场景规模相关**”的（小场景正常、大场景必挂），与 wrapper / `DISPLAY` / `PYTORCH_CUDA_ALLOC_CONF` / `--/rtx/raytracingEnabled=false` / 跳过 `enable_render(False)` **均无关**（这五项已逐一排除）| 验收协议暂定 **384 × 600**（230400 样本 = 原 512 的 75%）；需复核再定 1500 步（见坑 9c）|
| **9c** | **绝不能在训练批次进行中并发跑 eval** | 2026-09-14 实测：我把一个 eval 测试与 A2L3 的 2 个训练并发跑，显存撞车 ⇒ **eval 报 `torch.OutOfMemoryError`，同时把 A2L3 的 seed 11/12 一起挤爆**（`ckpt=NONE`）。20M 训练末期单进程会涨到 ~11 GiB，2 个训练 + 1 个 eval 必然超 32 GB | **一切 GPU 作业串行**：训练批次跑完、`nvidia-smi` 确认无 compute app 后，再跑 eval。另：“步数×env 数”也是显存受限的（`384×1500` 单跑也可能不够）|
| **9d** | 不同 `--num-envs` 会采到**不同的布局** | 同一 seed 下 `128/256/384/448` 的 `arrival@0.2` 不同（0.8516/0.8594/0.8359/0.8192）—— 因为布局采样依赖 RNG 流，而 RNG 消耗与 env 数相关 | **协议尺寸一旦冻结就全批统一**；跨尺寸**不可直接比较**，换尺寸必须重跑各批验收（**不需重训**）|
...
### 0.5.17 P1-A2L3 执行记录（**2026-09-14 18:24–18:39**）—— **双口径验收完成；1500 口径下 600 口径的 3 个"失败"全部翻盘**

**① 训练（✅ 3/3 完成）**：`A2L3` = `A2` 单键 `pillar_layers_range [2,6]→[3,3]`，L=3，**M = 4×3 = 12**，最坏相邻层间距 **0.792 × 2r ⇒ 连续**（CPU 门禁 PASS）。
...
- 3/3 `ok=True`、`worst exit=0`；显存 ~7.23 GiB/进程。
- ⚠️ seed 13 只有 353 s（vs 535/533 s）：首批 3 seed 里 11/12 被我并发跑 eval 时 OOM 杀掉，**只有 seed 13 独占 GPU 跑完**，因此它更快。**这不是 seed 13 更快收敛的证据，只是机器负载不同。**

**② 600 口径（对齐口径，`/tmp/navvel_p1/proto_a2L3_600/`）**

| seed | `arrival@0.2` ON | OFF | 依赖度 OFF/ON | 首达中位(ON) | 碰撞 | `drop_rel_step_frac`(ON) | `corr_mean`(ON) | 零介入率(ON) |
|---|---|---|---|---|---|---|---|---|
| 11 | 0.8411 | 0.7031 | 0.8359 | 438 | 0 | 0.0401 | 0.2183 | 0.5968 |
| 12 | 0.8073 | 0.6693 | 0.8291 | 426 | 0 | 0.0000 | 0.2282 | 0.5944 |
| 13 | 0.7891 | 0.7865 | 0.9967 | 421 | 0 | 0.0000 | 0.1867 | 0.6090 |
| **均值** | **0.8125** | 0.7196 | **0.8857** | 428.3 | 0 | **0.0134** | 0.2111 | 0.6001 |

门禁（聚合器）：`arrival@0.2≥0.85` **FAIL**；`filter_dependency≥0.95` **FAIL**；`zero_intervention≥0.95` FAIL；其余 PASS。
触发器：**`[TRIGGERED]` K 扩容规则**（`drop_rel_step_frac` = 0.0134 > 0.01）⇒ 按字面就该走**路线 A（升 K，红线）**。

**③ 1500 口径（收口口径，`/tmp/navvel_p1/proto_a2L3_1500/`）**

| seed | `arrival@0.2` ON | OFF | 依赖度 OFF/ON | 首达中位(ON) | 首达 p90(ON) | 碰撞 | `drop_rel_step_frac`(ON) | `corr_mean`(ON) | 零介入率(ON) |
|---|---|---|---|---|---|---|---|---|---|
| 11 | **1.0000** | 0.9896 | 0.9896 | 441 | 508.0 | 0 | 0.0160 | 0.0902 | 0.8331 |
| 12 | **0.9974** | 1.0000 | 1.0026 | 428 | 492.9 | 0 | 0.0000 | 0.0949 | 0.8306 |
| 13 | **0.9974** | 0.9974 | 1.0000 | 424 | 468.0 | 0 | 0.0000 | 0.0788 | 0.8349 |
| **均值** | **0.9983** | 0.9957 | **0.9974** | 431.0 | | 0 | **0.0053** | 0.0880 | **0.8329** |

门禁（聚合器，`/tmp/navvel_p1/proto_a2L3_1500/agg.json`）：

```
[VALUE] filter_dependency_OFF_over_ON = 0.9974
[PASS]  filter_dependency_gate>=0.95 = True      <-- 600 口径是 0.8857 FAIL
[FAIL]  zero_intervention_rate_gate>=0.95 = False   (0.8329)
[PASS]  h_min_train_gate>=0 = True
[PASS]  arrival@0.2_gate>=0.85 = True            <-- 600 口径是 0.8125 FAIL
[PASS]  zero_collision_gate = True
[PASS]  zero_oob_gate = True
[PASS]  stall_gate<=0.10 = True
[PASS]  dropped_relevant_frac_gate<0.01 = True
triggers: [not triggered] K escalation rule: dropped_relevant_step_frac(ON) = 0.0053
```

**④ 本轮最重要的方法学结论：600 步"时间窗"口径产生了 3 个假结果**

| 结论 | 600 口径 | 1500 口径 | 后果 |
|---|---|---|---|
| `arrival@0.2 ≥ 0.85` | 0.8125 ❌ | **0.9983 ✅** | 600 步把"能把 40% episode 跑完"当成能力 |
| `filter_dependency ≥ 0.95` | 0.8857 ❌ | **0.9974 ✅** | 600 步让人以为滤波器**拖累**了成功率 11.4 pt |
| **K 扩容触发器** | **TRIGGERED (0.0134)** | **not triggered (0.0053)** | ⚠️ **按 600 口径的字面规则会去升 K —— 撞阶段 2 的红线，而这是完全没必要的** |
| `zero_intervention_rate` | 0.6001 ❌ | 0.8329 ❌ | 唯一真的不过的项；但它**属于阶段 2**（见 §4.3） |

⇒ **`dropped_relevant_step_frac` 只能在收口口径下读**，否则会把"没跑完"误读成"看不见障碍"。
⇒ 这也解释了历史疑惑：A1b `0.8053` / A2 `0.8340` **全部是 600 口径**，所以它们与 0.85 门槛的"差 1.6 pt"根本不是能力差，**是时间窗差**。

**⑤ 阶段归属（务必分清，否则会把阶段 2 的失败读成阶段 1 的失败）**
按 §4.1/§4.2/§4.3：

| 门禁 | 所属阶段 | 1500 口径 A2L3 |
|---|---|---|
| `arrival@0.2 ≥ 0.85` | **阶段 1** | ✅ PASS (0.9983) |
| 0 碰 / 0 OOB | **阶段 1** | ✅ PASS |
| `h_min_train ≥ 0` | 阶段 2（也已接线） | ✅ PASS |
| `stall ≤ 10%` | 阶段 2 | ✅ PASS |
| `dropped_relevant_frac < 1%` | **阶段 1** | ✅ PASS (0.0001) |
| **`filter_dependency ≥ 0.95`** | **阶段 2** | ✅ **PASS (0.9974)** ← 原以为"离得很远" |
| **`zero_intervention_rate ≥ 0.95`** | **阶段 2** | ❌ FAIL (0.8329) ← **唯一未过项** |
| **`min d_min ≥ 0.10 m`** | **阶段 1** | ⚠️ **未接线，见 §0.5.17⑥** |

**⑥ ⚠️ 发现的计划-实现缺口：阶段 1 的安全余量门禁 `min d_min ≥ 0.10 m` 从未被强制执行**

- §4.1/§4.2 要求：`min d_min ≥ 0.10 m`（"全程最小表面净空"，§4.1 进一步说"相对**实物**，即 `box_net` 口径"）。
- **实测**：`min_clearance_global_min`（= 代码定义的表面净空 `d_i = ‖drone−p_oi‖ − (r_o + drone_radius + inflation)`）**六个 run 全在 0.0501–0.0867**，即 **< 0.10 m**。
- **但**：`aggregate_acceptance_eval.py` **没有**这条门禁（`min_clearance_global_min` 只在 `METRIC_ORDER` 里显示）；实际接线的是 `h_min_train ≥ 0`（= `min_clearance − cbf_extra`，实测 ≈ 0.0000 ✓）。
- 而且 §4.1 用来定义口径的 **`box_net` 在整个仓库里不存在**（`grep -rn box_net --include=*.py` 为空）⇒ "相对实物"到底要不要减掉 `drone_radius`，**计划里从未定死**。
- ⇒ **这是一个必须由你裁定的口径问题，不能由我猜**。三种读法与实测值的对应关系：
  - (a) `d_min = dist − r_o`（只减障碍物半径）⇒ 0.0501 + 0.12 = **0.170 m ≥ 0.10 ✅**
  - (b) `d_min = dist − r_o − drone_radius`（两个表面之间的真实间隙）⇒ 0.0501 + 0.02 = **0.070 m < 0.10 ❌**
  - (c) `d_min = dist − (r_o + drone_radius + inflation)`（= 代码现在的 `min_clearance`）⇒ **0.0501 < 0.10 ❌**
- 注意 (a)(b)(c) 的所有历史批次（A1a/A1b/A2/A2L2）**也是同样的情况**，所以这**不是 A2L3 引入的新问题**，而是从 K1 起就存在的口径空白。

---

### 0.5.18 同口径阶梯（**2026-09-14 18:39–18:54**）—— **发现两个大问题：历史阶梯是"跨尺寸混比"；收口口径下阶梯已饱和**

**① 先修一个一直存在的错误：历史验收是 512 envs，与 384 不可比**

| 批次 | 历史口径（实测自 log 的 `num_envs`/`rollout_steps`） |
|---|---|
| `eval_a1a` / `eval_a1b` / `eval_a2` | **512 envs × 600 steps** |
| A2L3 本轮 | 384 envs |

按 §0.5.7 **坑 9d**（不同 `--num-envs` 抽取不同障碍布局 ⇒ 不可比），所以 §0.5.14/§0.5.15 里"A1b `0.8053` → A2 `0.8340` → A2L2 `0.8359`"这条"阶梯"**是跨尺寸混比出来的**，差值里既含几何差异也含布局差异，**不可用于归因**。⇒ 本轮把 `A1a/A1b/A2` 在 **384** 下**重新验收**（**无需重训**，ckpt 未变），才第一次得到真正同口径的阶梯。

**② 同口径阶梯（384 envs，3 seed 均值，ON 侧）**

口径 **384 × 600**（对齐/时间窗）：

| profile | M | 几何 | `arrival@0.2` ON | OFF | 依赖度 | 首达中位 ON | 碰撞 ON | `drop_rel_step_frac` ON | `corr_mean` ON | 零介入率 ON |
|---|---|---|---|---|---|---|---|---|---|---|
| `A1a` | 16 | 连续 | 0.7873 | 0.6441 | 0.8181 | 430.7 | 0 | 0.0000 | 0.2537 | 0.6000 |
| `A1b` | 16 | 连续 | 0.7977 | 0.5885 | 0.7378 | 438.3 | 0 | **0.0295** ⚠️ | 0.2608 | 0.5800 |
| `A2` | 24 | **有缝**❌ | **0.8411** | 0.7161 | 0.8514 | 439.7 | 0 | **0.1441** ❌ | 0.2197 | 0.5993 |
| `A2L3` | 12 | 连续 | 0.8125 | 0.7196 | 0.8857 | 428.3 | 0 | **0.0134** ⚠️ | 0.2111 | 0.6001 |

口径 **384 × 1500**（收口/完整 episode）：

| profile | M | 几何 | `arrival@0.2` ON | OFF | 依赖度 | 首达中位 ON | 碰撞 ON | `drop_rel_step_frac` ON | `corr_mean` ON | 零介入率 ON |
|---|---|---|---|---|---|---|---|---|---|---|
| `A1a` | 16 | 连续 | 0.9991 | 0.9818 | 0.9826 | 435.0 | 0 | **0.0000** ✅ | 0.1088 | 0.8292 |
| `A1b` | 16 | 连续 | 0.9974 | 0.9661 | 0.9686 | 441.7 | 0 | **0.0118** ⚠️ | 0.1103 | 0.8223 |
| `A2` | 24 | **有缝**❌ | **1.0000** | 0.9939 | 0.9939 | 441.7 | 0 | **0.0838** ❌ | 0.0910 | 0.8337 |
| `A2L3` | 12 | 连续 | 0.9983 | 0.9957 | 0.9974 | 431.0 | 0 | **0.0053** ✅ | 0.0880 | 0.8329 |

**③ ⚠️ 大问题一：收口口径下"阶梯已饱和"，`arrival@0.2` 失去区分度**

- 1500 口径下四个阶梯的 `arrival@0.2` 全在 **0.9974–1.0000**，**全部远超门禁 0.85** ⇒ **阶段 1 的到达率门禁对任何阶梯都不再有区分力**，它只能当"必要条件"用。
- `filter_dependency` 同样饱和：**0.9686–0.9974，四个全 PASS** ⇒ §4.3 原以为"离得很远"的依赖度门禁，**实际是全部通过**。
- 连新加的**速度指标也饱和**：首达中位 **431–442 步**，四个阶梯几乎一样 ⇒ 它对**减速**敏感，但对本阶梯**不敏感**。
- ⇒ **在一个完整 episode 里，这四个世界对当前策略都太容易了。** 600 步的"时间窗"无意中充当了**难度放大器**（它实际测的是速度），这就是为什么 600 口径看起来"能区分"、而 1500 口径一片饱和。

**④ 大问题二：唯一还有区分度的轴是"观测容量"，而且它把 `A1b`/`A2` 单挑出来了**

1500 口径下按 K 扩容触发器（`drop_rel_step_frac > 0.01`）分层：

| profile | M > K? | `drop_rel_step_frac` @1500 | K 扩容触发 | 读法 |
|---|---|---|---|---|
| `A1a` | 16 > 8 | 0.0000 | **未触发** ✅ | 柱细（r=0.354）⇒ 进入 `danger_radius` 的层少 ⇒ 8 槽够 |
| `A2L3` | 12 > 8 | 0.0053 | **未触发** ✅ | 层级少 ⇒ 8 槽够 |
| `A1b` | 16 > 8 | 0.0118 | **触发** ⚠️ | 柱粗（r=0.4243）⇒ 相关层变多 ⇒ 8 槽不够 |
| `A2` | 24 > 8 | 0.0838 | **触发** ❌ | 层级最多 + 逐柱 z 跨度 ⇒ 彻底不够 |

⇒ **"槽位粒度"（= 路线 C 要解决的问题）是本轮唯一仍然活跃的区分轴**，而且它**独立于几何难度**：`A1b` 与 `A1a` 层数相同（都是 16），只因为**柱半径大 20%**，相关层就多到超出 K=8。这是一个**之前完全没被识别出来**的现象。
⇒ 这也说明 **`A2` 的"最好"（`arrival@0.2` = 1.0000）毫无意义**：它是在一个柱中间有缝、且 8.4% 的步"相关障碍看不见"的世界里取得的。

**⑤ 阶段 1 门禁逐项（收口口径 384×1500，ON 侧）**
...
| 0 OOB | ✅ 0 | ✅ 0 | ✅ 0 | ✅ 0 |
| `h_min_train ≥ 0` | ✅ | ✅ | ✅ | ✅ |
| `dropped_relevant_frac < 1%` | ✅ | ✅ | ✅ 0.0036 | ✅ |
| **`min d_min ≥ 0.10 m`** | ⚠️ **未接线**（见 §0.5.17⑥） | ⚠️ | ⚠️ | ⚠️ |

**⇒ 四个阶梯在收口口径下全部通过阶段 1 ⇒ 阶段 1 的门禁"过了，但含金量为零"**，因为它无法区分任何两个世界。

**⑥ ON/OFF 碰撞拆分（澄清：所有碰撞都在 OFF 侧）**

| 口径 | profile | 碰 ON | 碰 OFF |
|---|---|---|---|
| 600 | `A1a` | 0 | 1 |
| 600 | `A1b` | 0 | 1 |
| 600 | `A2` | 0 | 0 |
...
⇒ **ON 侧 8 个批次无一例外 0 碰 / 0 OOB**；聚合器的 `zero_collision_gate` 判 FAIL 是因为它**同时要求 OFF 侧为 0**，而 OFF 侧的碰撞**恰是滤波器存在的证据**、是 §4.3 阶段 2 的考查项，**不是阶段 1 的失败**。这一点在比较门禁时必须分清，否则会把阶段 2 的预期现象误读成阶段 1 的失败。
...
11. **✅ 已定（2026-09-14 18:3x，用户决策）：验收协议 = 两套都跑**（见 §0.6.6）：
    - **`384×600`** 用于与历史数字对齐；**`384×1500`** 用于判过/不过（= 每 episode 成功率）；
...
| # | 决策项 | 用户决定 | 落地含义 |
|---|---|---|---|
| 1 | **验收协议** | **两套都跑** | 每个 ckpt 跑 **2 次** eval（filter ON/OFF 各 1）：<br>• **`384 × 600`** —— 与历史数字（A1b `0.8053` / A2 `0.8340` / A2L2 `0.8359` / A2L3-s13 `0.7943`）**同口径**，用于**阶梯对齐**；<br>• **`384 × 1500`** —— **收口口径**（= 1 个完整 episode），`arrival@0.2` 读作"**每 episode 成功率**"，用于**判过/不过**。<br>⚠️ **两个口径的数字永远不许混进同一张对照表**；报告必须写清 `rollout_steps`。 |
