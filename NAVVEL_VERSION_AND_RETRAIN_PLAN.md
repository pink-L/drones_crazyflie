...
#### 0.6.8 A2L3 与历史阶梯的同口径对照（**512×600**，2026-09-14）

| 档 | profile | M | 柱连续性 | `arrival@0.2` (ON) | `arrival_steps_median` (ON/OFF) | 碰撞 ON/OFF | `dropped_relevant_step_frac` (ON) |
|---|---|---|---|---|---|---|---|
| A1a | `A1a` | 16 | — | 0.7865 | （重跑中） | — | 0.0000 |
| A1b | `A1b` | 16 | ✅ 0.531 | 0.8053 | （重跑中） | — | 0.0295 |
| A2 | `A2` | 24 | ❌ 1.443 | 0.8340 | （重跑中） | — | 0.1886 |
| **A2L3** | `A2L3` | **12** | ✅ 0.792 | **0.8034** | **428.7 / 392.3** | **0 / 1** | **0.0001** |

> A1a/A1b/A2 的历史行是 2026-09-12 的旧 log（**无速度指标、无 1500 列**），正按 `512×{600,1500}` 重跑（**无需重训**）。
> ⚠️ `A2` 的 0.8340 是在**几何非法**（柱中有 1.44×2r 的缝）的世界里取得的，**不可作为合法阶梯点**（§0.5.16⑥）。
> ✅ **`A2L3` 的意义**：它是**唯一同时满足"L≥3 连续柱"与"M ≤ K"**的档（§0.5.16⑥ 已证明两者只能在 L=3 同时成立），且 `dropped_relevant_step_frac` = **0.0001**（vs A2 的 0.1886）⇒ **连续柱把观测窗口的浪费消掉了 3 个数量级**。这是**路线 B 真正成立的证据**。
