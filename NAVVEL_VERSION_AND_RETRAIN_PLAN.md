...
**⑦ 验收（进行中）**：`512×600`（10 次）+ `384×1500`（10 次），全部串行，产物 `/tmp/navvel_p1/acc_a4_{512x600,384x1500}/`。
判定分两层：(a) **确定性与复用核对** —— seed 11–13 应与 A3 逐字段一致；(b) **交付门槛** —— 5 seed 在 `384×1500` 下通过 §4.2 全部 7 个门槛。
...
⇒ **`A3` 在三个判别量上全部最优**，且**碰撞双侧为 0**。`A1a` 的 `drop_step = 0` 是**因为它根本没有柱子**（自由球全被移除），不是因为它更好。
